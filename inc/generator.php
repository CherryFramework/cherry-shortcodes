...
class Cherry_Shortcodes_Generator {
	...
	public static function reset() {
		...
		foreach ( array_keys( (array) Cherry_Shortcodes_Data::shortcodes() ) as $shortcode ) delete_transient( 'cherry_shortcodes/generator/settings/' . $shortcode );
	}
	...
	public static function popup() {
		...
		if ( $output && CHERRY_SHORTCODES_ENABLE_CACHE ) echo $output;
		// Cache not found
		else {
			ob_start();
?>
			...
			<div id="cherry-generator">
				<div id="cherry-generator-header">
					...
					<div id="cherry-generator-filter">
						...
						<?php foreach ( (array) Cherry_Shortcodes_Data::groups() as $group => $label ) echo '<a href="#" data-filter="' . $group . '">' . $label . '</a>'; ?>
					</div>
	...
	public static function settings() {
		self::access();
		// Param check
		if ( empty( $_REQUEST['shortcode'] ) ) wp_die( __( 'Shortcode not specified', 'cherry-shortcodes' ) );
		// Get cache
		...
		// Cache not found
			...
			if ( isset( $shortcode['note'] ) || isset( $shortcode['example'] ) ) {
				...
				if ( isset( $shortcode['example'] ) ) $return .= wpautop( '<a href="' . admin_url( 'admin.php?page=cherry-shortcodes-examples&example=' . $shortcode['example'] ) . '" target="_blank">' . __( 'Examples of use', 'cherry-shortcodes' ) . ' &rarr;</a>' );
				$return .= '</div></div>';
			}
			// Shortcode has atts
			...
			if ( $shortcode['type'] == 'single' ) $return .= '<input type="hidden" name="cherry-generator-content" id="cherry-generator-content" value="false" />';
			// Wrapping shortcode
			else {
...
<div class="cherry-generator-presets alignright" data-shortcode="<?php echo sanitize_key( $_REQUEST['shortcode'] ); ?>">
	...
	public static function presets_list( $shortcode = false ) {
		...
		else echo sprintf( '<b>%s</b>', __( 'Presets not found', 'cherry-shortcodes' ) );
	}
